...
def generate_log_html():
    """Generate log.html dynamically from the last 150 entries in loop.log."""
    log_html_path = os.path.join(WORKING_DIR, "log.html")
    now = datetime.datetime.now()
    now_str = now.strftime("%Y-%m-%d %H:%M MST")

    # Read last 150 lines from loop.log
    try:
        with open(LOOP_LOG, "r") as f:
            lines = f.readlines()
        recent_lines = lines[-150:] if len(lines) > 150 else lines
    except Exception as e:
        log(f"generate_log_html error: {e}")
        return
    ...
    for line in recent_lines:
        ...
        # Extract just time (HH:MM:SS) from timestamp
        try:
            ts_time = ts_full.split()[1]  # "2026-03-06 19:54:12" -> "19:54:12"
        except:
            ts_time = ts_full
    ...
    for entry in log_entries:
        # Add spacing before loop markers for readability
...
<p class="intro">Live view of loop.log — the raw operational record of what the daemon does between context windows. Shows the last 150 entries: loops, email checks, heartbeats, autonomous sessions, errors, restarts. The record between the records. This page updates automatically every heartbeat.</p>
