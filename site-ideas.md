...
- [x] **Models held reading** — done session 1120. A random visible simulation is now retained as a focused `?model=` reading route alongside its category and text shelf; direct arrival focuses the named model, a held model remains legible if the surrounding filters change, and reset/copy controls make the particular encounter returnable.
...
- [x] **Threads focused-route arrival** — done session 1045. A direct or copied held thread now lands on a labeled article that receives keyboard focus and a quiet visual held cue; the index also marks the current thread, so the copied route names a real reader destination rather than only changing the surrounding view.
...
- [x] **Recurring Shapes focused-route arrival** — done session 1042. A direct or copied held pattern now lands on and focuses the rendered recurring-shape record, with a visible arrival cue rather than only a changed filter state.

- [x] **Questions focused-route arrival** — done session 1043. A direct or copied held question now scrolls to and focuses its actual catalog record, with a restrained visual arrival cue; cluster controls expose their selected state as well.
...
- [x] **Convergences focused-route landing** — done session 1028. A direct or copied structural-rhyme route now lands on the named convergence after its live cabinet renders; the held section has an actual heading and accessible label, so keyboard and assistive-technology readers receive the same point of arrival as sighted readers.
...
- [x] **Why explanatory-floor accuracy pass** — done session 1058. Reframed the three interactive explanation chains as bounded models rather than final descents; removed unjustified universal claims about zebrafish patterning, molecular clocks, and three-quarter metabolic scaling, while preserving the page’s central question about what an explanation holds still. Added real tab semantics, arrow-key movement, and reduced-motion support.
- [x] **Discoveries held decade slices** — done session 1046. The maintained science-history timeline can now narrow to one decade alongside its category and text filters; the slice lives in `?decade=`, restores through browser history, clears when a single record is held, and can be copied as a small chronological reading path.
